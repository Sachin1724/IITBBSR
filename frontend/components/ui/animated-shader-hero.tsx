...
void main(){gl_Position=position;}`;
    ...
    class PointerHandler {
        ...
        private moves = [0, 0];
        // Define first property with getter
...
const Hero: React.FC<HeroProps> = ({
    ...
    // Color theme constants (Purple/Cosmic theme)
    // Replaced Orange/Yellow with Cosmic/Lavender
    ...
    return (
      ...
      `}</style>
            ...
            <canvas
                ref={canvasRef}
                className="absolute inset-0 w-full h-full object-contain touch-none opacity-40"
                style={{ background: '#070F2B' }} // Cosmic deep background
            />
            ...
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
                ...
                {trustBadge && (
                    <div className="mb-8 animate-fade-in-down">
                        <div className="flex items-center gap-2 px-6 py-3 bg-cosmic-dark/40 backdrop-blur-md border border-cosmic-lavender/30 rounded-full text-sm">
                            {trustBadge.icons && (
                                <div className="flex">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10 animate-fade-in-up animation-delay-800">
                            {buttons.primary && (
                                <InteractiveHoverButton
                                    ...
                                    className="w-64 py-6 text-xl font-doto tracking-wide border-cosmic-lavender/50 hover:border-cosmic-lavender"
                                />
                            )}
                            {buttons.secondary && (
                                <button
...
* Modified for Cosmic Watch Theme
*/
precision highp float;
out vec4 O;
...
void main(void) {
    ...
    // Cosmic Theme Colors
    // Tinting the background noise to be more blue/purple
    ...
    for (float i=1.; i<12.; i++) {
        ...
        // Modified color palette for stars/particles
        // Blue/Purple/Pink spectrum
		col+=.00125/d*(cos(sin(i)*vec3(0.5,0.2,1.0))+1.); // Cosmic Purple/Blue base
		
        float b=noise(i+p+bg*1.731);
		col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
        
        // Mix with cosmic background colors
		col=mix(col,vec3(bg*.15,bg*.10,bg*.25),d); // Darker blue/purple mix
	}
	O=vec4(col,1);
}`;
